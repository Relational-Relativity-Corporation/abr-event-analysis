...
FM2 - Relational Isolation: adj+(e) = empty for all e
Circulation Cancellation - inadmissibility signal, not a legitimate failure mode.
"""
...
class FailureMode(Enum):
    ...
    CIRCULATION_CANCELLATION = "circulation_cancellation"


def detect_failure_mode(delta_field, sigma_field, edges, rho_base=0.2):
...
def expression_condition(delta_field, sigma_field, edges, rho_base=0.2):
    rank_sigma, _ = im_sigma_rank(sigma_field)
    asym = antisymmetric_term(delta_field, edges, rho_base)
